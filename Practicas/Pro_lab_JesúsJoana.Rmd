...
# Generar tiempos de fallo para cada VM
D1 <- rexp(num_simulations, rate = lambda)
D2 <- rexp(num_simulations, rate = lambda)
D3 <- rexp(num_simulations, rate = lambda)

# Calcular T_fallo como el máximo de D1, D2 y D3 para cada simulación
shutdown_sims <- function(){
  pmax(D1, D2, D3)
}

resultado <- shutdown_sims()
head(resultado)
...
# Dibujar el histograma de shutdown_sims como densidad
hist(resultado, probability = TRUE, breaks = 100, 
     main = "Histograma de la distribución de T_fallo",
     xlab = "T_fallo", ylab = "Densidad", col = "lightblue",
     border = "blue")
...
curve(3 * (1 - exp(-x))^2 * exp(-x), from = 0, to = max(resultado),
